...
export default function Home() {
  ...
  async function upload(event: ChangeEvent<HTMLInputElement>) {
    ...
    const remainingCapacity = 20 - receiptItems.length;
  ...
  async function resubmit(expense: Expense) {
    ...
    if (receiptWasRequested && receiptItems.length === 0) {
      setError("Upload the requested replacement receipt bundle above before resubmitting");
      return;
    }
    setBusy(true);
    try {
      await api(`/expenses/${expense.id}/resubmissions`, identity, { method: "POST", body: JSON.stringify({ purpose, items: receiptWasRequested ? receiptItems.map((item) => ({ receipt_id: item.receipt.id, merchant: item.merchant, amount_minor: Math.round(Number(item.amount) * 100), currency: "INR", incurred_date: item.date })) : null, items_mode: "APPEND" }) });
      if (receiptWasRequested) { receiptItems.forEach((item) => URL.revokeObjectURL(item.preview)); setReceiptItems([]); }
      await refresh();
    }
